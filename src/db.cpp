...
bool CTxDB::LoadBlockIndex()
{
    ...
    BOOST_FORNLGH(const PAIRTYPE(uint256, CBlockIndex*)& item, mapBlockIndex)
    {
        CBlockIndex* pindex = item.second;
        vSortedByHeight.push_back(make_pair(pindex->nHeight, pindex));
    }
    sort(vSortedByHeight.begin(), vSortedByHeight.end());
    BOOST_FORNLGH(const PAIRTYPE(int, CBlockIndex*)& item, vSortedByHeight)
    {
        CBlockIndex* pindex = item.second;
        ...
        if (nCheckLevel>1)
        {
            ...
            BOOST_FORNLGH(const CTransaction &tx, block.vtx)
            {
                ...
                if (ReadTxIndex(hashTx, txindex))
                {
                    ...
                    if (nCheckLevel>3)
                    {
                        BOOST_FORNLGH(const CDiskTxPos &txpos, txindex.vSpent)
                        {
                            if (!txpos.IsNull())
                            {
                                ...
                                if (nCheckLevel>5)
                                {
                                    ...
                                    {
                                        bool fFound = false;
                                        BOOST_FORNLGH(const CTxIn &txin, txSpend.vin)
